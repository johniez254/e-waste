<?php
	...
	*/
	 ...
	 *     $data['transaction_date'], $data['item'], $data['batch'], $data['section'], $data['transaction_type'], $data['quantity']
	...
	*/
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['transaction_date'], $data['item'], $data['batch'], $data['section'], $data['transaction_type'], $data['quantity']
	...
	*/
	...
	function update_balances($data){
		...
		*/
		$positive_transactions = "'Incoming'";
		
		/* get sum of all item transactions and update item balance */
		if($item){
			$item_balance = sqlValue("select sum(if(transaction_type in ({$positive_transactions}), quantity, -1 * quantity)) from transactions where item={$item}");
			sql("update items set balance='{$item_balance}' where id={$item}", $eo);
		}
		
		/* get sum of all item transactions and update item balance */
		if($batch){
			$batch_balance = sqlValue("select sum(if(transaction_type in ({$positive_transactions}), quantity, -1 * quantity)) from transactions where batch={$batch}");
			sql("update batches set balance='{$batch_balance}' where id={$batch}", $eo);
		}
	}